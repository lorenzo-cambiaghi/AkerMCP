...
- **With AkerMCP (and LynxMCP):** 
  ...
  5. **Visual A/B Testing:** Finally, the AI didn't just assume it worked. It used `execute` to spawn two identical test objects in the scene—one with a standard shader, and one with the newly generated Voxel GI shader. It then used `take_screenshot` to visually compare them side-by-side, proving to the user that the custom Global Illumination was contributing correctly to the mesh.
